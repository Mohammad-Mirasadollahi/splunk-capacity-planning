...
window.SCP_TIPS = {
  en: {
    ...
    available_summaries_gb: {
      title: "[volume:_splunk_summaries] maxVolumeDataSizeMB",
      formula: "available_summaries_gb → summaries/DMA volume cap",
      body: "Budget for the summaries volume used by tstatsHomePath / summary indexes. Keep on SSD; path is on Paths.",
      example: "If available < need → Calculate errors (hard budget).",
      links: [
        { label: "Reference hardware", url: "https://docs.splunk.com/Documentation/Splunk/latest/Capacity/Referencehardware" },
      ],
    },
    ...
    "summaries need GB": {
      ...
      formula: "Σ summary index maxTotalMB / 1024\n+ DMA_GB (ES: Daily_Raw × 3.4 × dma_years when dma_pct=0)",
      body: "Budget for volume:summaries — summary indexes plus DMA/tstatsHomePath. ES official DMA uses raw ingest × 3.4/year, not a percent of searchable on-disk. Keep on SSD/NVMe separate from cold.",
      example: "100 GB/day ES, dma_years=1 → ~340 GB DMA + any summary index caps.",
      links: [
        { label: "ES DMA storage and retention", url: "https://help.splunk.com/en/splunk-enterprise-security-8/install/8.6/installation/configure-data-models-for-splunk-enterprise-security" },
        { label: "Reference hardware", url: "https://docs.splunk.com/Documentation/Splunk/latest/Capacity/Referencehardware" },
      ],
    ...
    "Summaries need (GB · total)": {
      ...
      formula: "Σ summary caps + DMA (Daily_Raw × 3.4 × dma_years when ES official)",
      body: "Cluster-wide volume:summaries budget including DMA/tstats and optional summary indexes.",
      example: "100 GB/day ES → 340 GB DMA at dma_years=1 before summary indexes.",
      links: [
        { label: "ES DMA storage and retention", url: "https://help.splunk.com/en/splunk-enterprise-security-8/install/8.6/installation/configure-data-models-for-splunk-enterprise-security" },
      ],
    },
...
const SCP_TIP_IMPACTS = {
  en: {
    ...
    summary_retention_days: "Longer summary retention → more disk on volume:_splunk_summaries. Shorter → summary indexes shrink and age out sooner.",
    ...
    summaries_path: "Sets [volume:_splunk_summaries] path=. Putting it on cold HDD → acceleration and summary search suffer.",
    ...
    available_summaries_gb: "Raises [volume:_splunk_summaries] maxVolumeDataSizeMB. Below need → Calculate errors.",
    ...
    "summaries need GB": "Grows with summary indexes, dma_years (ES ×3.4/year), or dma_pct override — not index retention_days unless using override mode.",
    ...
    "Summaries need (GB · total)": "Same as summaries need — ES DMA uses daily_raw × 3.4 × dma_years when dma_pct=0.",
  ...
  fa: {
    ...
    summary_pct: "درصد summary را بالا ببرید → ingest summary و دیسک volume:_splunk_summaries بیشتر می‌شود. کمش کنید → ایندکس‌های summary کوچک‌تر می‌شوند.",
    ...
    summaries_path: "مقدار [volume:_splunk_summaries] path= را می‌گذارد. گذاشتن روی HDD cold → شتاب‌دهی و سرچ summary ضعیف می‌شود.",
    ...
    available_summaries_gb: "سقف [volume:_splunk_summaries] maxVolumeDataSizeMB را بالا می‌برد. کمتر از نیاز → خطای Calculate.",
    ...
    "summaries need GB": "با summary، dma_years (ES ×۳٫۴/year) یا override dma_pct زیاد می‌شود — نه retention ایندکس مگر حالت override.",
    ...
    "نیاز Summaries (GB · total)": "همان summaries — DMA رسمی ES: daily_raw × ۳٫۴ × dma_years وقتی dma_pct=۰.",
...
(function localizeFa() {
  ...
  const map = {
    ...
    available_summaries_gb: ["volume:_splunk_summaries", "بودجه volume summaries/DMA."],
  ...
  [
    ...
    ["Summaries need (GB · total)", "volume:summaries شامل DMA رسمی ES (×۳٫۴/year) و summary indexes."],
    ...
    ["نیاز Summaries (GB · total)", "volume:summaries شامل DMA رسمی ES (×۳٫۴/year) و summary indexes."],
